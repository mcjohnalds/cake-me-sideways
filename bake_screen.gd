...
func _stop_thermometer() -> void:
	...
	layer_cooked_amounts.append(_thermometer_fill_ratio)
...
func _update_ui() -> void:
	for i in _MAX_LAYER_COUNT:
		...
		var layer_is_cooking := (
			_thermometer_is_raising and i == layer_cooked_amounts.size()
		)
		pan.visible = (
	...
	var cooked_all_layers := (
		layer_cooked_amounts.size() == _ORDER_LAYER_COUNT
	)
